...
const actions = {
    loadCharacters({ commit }) {
        ...
        const regexL = /[a-zA-ZöüóőúéáűíÖÜÓŐÚÉÁŰÍ]/;
    ...
    updateCurrentCharacters({ commit }) {
        ...
        for (let i = state.nextBatchPosition; currentCharacters.length < 8 * 20; i++) {
            currentCharacters.push({
                ...
                status: state.characters[i].status
            })
        }
    ...
    keyPressed(context, key) {
        if (state.currentCharacters[state.currentPosition].char === key) {
            if (state.currentCharacters[state.currentPosition].status === Status.NEXT) {
                ...
                if (state.currentCharacters[state.currentPosition].char === ' ') {
                    context.commit('setMissedSpace');
                }
            }

            while (state.currentPosition + 1 < state.currentCharacters.length) {
                if (state.currentCharacters[state.currentPosition + 1].status === Status.PLACEHOLDER) {
                    context.commit('setCurrentPosition', state.currentPosition + 1);
                }
                else {
                    context.dispatch('updateStatus', { 
...
const mutations = {
    ...
    setMissedSpace: (state) => (state.currentCharacters[state.currentPosition].char = '_'),
